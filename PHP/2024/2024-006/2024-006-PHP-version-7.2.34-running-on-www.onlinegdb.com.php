...
class GuardPatrolLoopDetector
{
    ...
    {
        $filePath = "input.txt"; // Specify the path to your input file here
        try {
     ...
     */
    private static function countDistinctPositionsVisited($filePath)
    {
        // Parse the grid
        $grid = self::parseGrid($filePath);

        // Find the guard's starting position and direction
        list($guardPos, $guardDir) = self::findGuard($grid);
     ...
     * @throws Exception If the file cannot be read.
     */
    private static function countObstructionPositions($filePath)
    {
        // Start total timing
     ...
     */
    private static function parseGrid($filePath)
    {
        ...
        while (($line = fgets($file)) !== false) {
            ...
            if ($trimmedLine !== '') {
                // Replace mb_str_split with str_split for compatibility with older PHP versions
                $gridList[] = str_split($trimmedLine);
            }
        }
        fclose($file);
     ...
     */
    private static function findGuard(&$grid)
    {
     ...
     */
    private static function getPossibleObstructions($grid, $guardPos)
    {
        $possible = [];
        foreach ($grid as $r => $row) {
            foreach ($row as $c => $cell) {
     ...
     */
    private static function simulateMovement($grid, $startPos, $startDir)
    {
        $visitedStates = [];
...
class State
{
    ...
    public function __construct($r, $c, $dir)
    {
        $this->row = $r;
        $this->col = $c;
        $this->direction = $dir;
